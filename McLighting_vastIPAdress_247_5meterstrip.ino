...
void setup() {
 ...
 // mode = OFF;
  ...
  wifiManager.setSTAStaticIPConfig(IPAddress(192,168,2,214), IPAddress(192,168,2,254), IPAddress(255,255,255,0));
  ...
  // ***************************************************************************
    ...
    if (mqtt_host != "" && String(mqtt_port).toInt() > 0) {
        snprintf(mqtt_intopic, sizeof mqtt_intopic, "%s/in", HOSTNAME); // aanpassing naar verwachtte topic in domoticz?
        snprintf(mqtt_outtopic, sizeof mqtt_outtopic, "%s/out", HOSTNAME);
  
  //    snprintf(mqtt_intopic, sizeof mqtt_intopic, "%s/in", HOSTNAME); // domoticz/in/ oude regel 
  //    snprintf(mqtt_outtopic, sizeof mqtt_outtopic, "%s/out", HOSTNAME);
  ...
  server.on("/merry_christmas", []() {
    exit_func = true;
    mode = MERRY_CHRISTMAS;
    getArgs();
    getStatusJSON();
  });

  server.on("/twinkle_fade", []() {
    exit_func = true;
    mode = TWINKLE_FADE;
    getArgs();
    getStatusJSON();
...
void loop() {
 ...
 if (mode == MERRY_CHRISTMAS) { //new xmas
    strip.setMode(FX_MODE_MERRY_CHRISTMAS);
    mode = HOLD;
  }
